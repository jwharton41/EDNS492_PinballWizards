/* 
    Secondary Playfield(V2) Program for Project Pinball
    Created by the Pinball Wizards
    S24-5131 Pinball Wizards - Project Pinball

    This is a secondary program the team used for the final playfield of project pinball. This code is the program that is currently uploaded on the Arduino Mega. 
    Gameplay begins with the player hitting the engage or start button, releasing the ball in front of the plunger. This program differs from the final playfield program
    as instead of the rollover switch in the gutter taking away a life, you must hit the start button to release the ball back in front of the plunger, which will take away a life.
    The game will end when the engage button has been hit 4 times(once at the beginning and 3 times during the game). 
 */
...
void loop() {
  ...
  switch(action) {
    ...
    case INPLAY: // state for gameplay occuring (game is happening)
      ...
      // Start button taking away a life
      Trigger1 = digitalRead(startPin); // Read the button state
      if(Trigger1 == 0) { // Check if the button is pressed (LOW for pull-up)
        lives -= 1; // subtract 1 life from the total
        Trigger1 = 1; // reset trigger1 to be 1 again
        myServo.write(90); // Rotate servo to move the ball into the gutter
        delay(1000); // delay to allow action to fully happen before restting the servo
        myServo.write(45); // Rotate servo back to 0 degrees
      }
